...
const Dashboard: React.FC = () => {
  const { currentTenant, tenants, isGlobalView } = useTenant();
  ...
  const stats = isGlobalView ? [
    ...
    {
      title: 'Total Residents',
      value: currentTenant?.totalResidents.toLocaleString() || '0',
      icon: Users,
      color: 'bg-blue-500',
      change: '+12 this month'
    },
    {
      title: 'Pending Dues',
      value: `₹${currentTenant?.pendingDues.toLocaleString() || '0'}`,
      icon: CreditCard,
      color: 'bg-green-500',
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            {isGlobalView 
              ? `Managing ${tenants.length} societies with ${globalStats.totalResidents} residents`
              : `Here's what's happening at ${currentTenant?.name || 'your society'} today.`
            }
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">
            {isGlobalView ? 'Global Revenue Overview' : 'Monthly Collection Overview'}
          </h2>
            ...
            {monthlyData.map((data, index) => (
              <div key={index} className="flex flex-col items-center flex-1">
                <div
                  ...
                  style={{
                    height: `${(data.amount / Math.max(...monthlyData.map(d => d.amount))) * 200}px`
                  }}
                ></div>
